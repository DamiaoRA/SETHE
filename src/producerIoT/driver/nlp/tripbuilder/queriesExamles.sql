...
asp_poi = ^torridipisa;chiesedipisa;palazzidipisa$
asp_move= ;Walk Bus;



^torridipisa(.*)palazzidipisa$

asp_poi=(^(Residence));((College & University)|(Food));((Arts & Entertainment )$);
asp_weather= Clouds; Clear; Clear
asp_rating= 7;8;8
asp_day=Monday;Thursday;Wednesday
asp_price=3;4;3

select traj_fk , regexp_matches("values", '^([a-z]*;*torridipisa[a-z]*;*) (.*chiesedipisa.*) ([a-z]*;*palazzidipisa[a-z]*;*)$'), values from tripbuilder.tb_category 
where traj_fk in (
select traj_fk from
(select traj_fk, regexp_matches("values", '([a-z]+ [a-z]+ [a-z]+ [a-z]+ [a-z]+ Walk Bus)'), values from tripbuilder.tb_move) as tm
)


Taxi Taxi Bus Bus Subway Subway Walk Walk Bus Bus Walk Walk
Walk Walk Walk Walk Walk Walk Bus Bus Bus Bus
