...
/**
 * Uma expressão é uma anotação em liguagem regular com o objetivo de encontrar
 * um ponto ou conjunto de pontos de uma trajetória.
 *
 */
public class Expression {
	...
	public void addAspect(String aspName, String value) {
		...
		value = value.replaceAll(Constant.REPEATED_PATTERN, "").trim();
		exp.setValue(value);
		exp.setOrder(order);
	...
	public String getRegexPoi() {
		if(getValuePoi() == null)
			return null;
//		if(getValuePoi().trim().isEmpty())
		if(getValuePoi().trim().equals("."))
			return "(.)*";
		return "(" + getValuePoi() + ")";
	}
	...
	public String getRegexCategory() {
		if(getValueCategory() == null)
			return null;
		if(getValueCategory().trim().equals("."))
			return "(.)*";
...
//	public boolean valuePoiIsEmpty() {
//		if (getValuePoi() != null && getValuePoi().equals("."))
//			return true;
//		return false;
//	}
	...
	public Double distance(String aspectType, String aspectValue, String function, Double limit, Double weight, PoI p1, PoI p2, Trajectory trajectory) {
		...
		if(idealValue.equals(Constant.ANY_VALUE))
			return 1d;

		double result = 0d;
		
		if(trajectory.getId().equals("http://localhost:8080/resource/TP2522"))
			System.out.println("Expression.distance()");
...
//			if(aspect.charAt(0) == '-')
			if(aspect.trim().startsWith(Constant.REPEATED))
				return true;
		}
		return false;
	}
